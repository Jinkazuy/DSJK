			...
			<view class="scroll-con" :style="{width: scrollWid}">
				...
				<view class="bottomNum" :style="{'paddingLeft': allNumLeft}">
					<text class="allNum" :style="{width: (maginL + 2) * 10 + 'px',left: -((maginL + 2) * 5) + 'px'}" v-for="(item,index) in scrollNumList" :key="index">
						<!-- 小数点关键在这里，父级设置10-50，那么这里就会渲染1-5，但实际选择时还是10-50，那么在父级同样需要换算一下 -->
						<!-- 父级比如：<div class="num-wrapper"><span class="num">{{userWeight/10 + (userWeight%10===0?'.0':'')}}</span>KG</div> -->
						<!-- 这个userWeight就是这个滑尺组件返回的数值，父级拿到后除以10即可得到小数 -->
						{{item/10}}
					</text>
				</view>
			</view>
