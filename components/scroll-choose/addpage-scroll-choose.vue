<template>
	<view class="scroll-choose-all">
		...
		<scroll-view class="scroll-choose" 
			scroll-x="true" upper-threshold="5" 
			lower-threshold="5" 
			...
			<view class="scroll-con" :style="{width: scrollWid}">
				<view class="topLine">
					<view 
						...
						:class="['allLine',item.type,
							editId!==1?
							index<(rulerLow-scrollStart)?'allLine-low':
							index>(rulerHig-scrollStart)?'allLine-hig':'allLine-nor':'']" 
						:key="index">
					</view>
				</view>
				<view class="bottomNum" :style="{'paddingLeft': allNumLeft}">
					...
					:class="[
						'allNum',
						item<=rulerLow&&editId!==1?'allNum-low':item>=rulerHig&&editId!==1?'allNum-hig':'allNum-nor'
					]" 
